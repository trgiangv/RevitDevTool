...
- net48 Test Explorer "could not be discovered": `CreateTestSession` failed to load `Unsafe` 6.0. Testhost BCL comes from the consumer `Microsoft.Testing.Platform.MSBuild` graph plus `AutoGenerateBindingRedirects`; the adapter ILRepacks its own copy and does not ship loose 3rd-party DLLs.
