...
Host `RunBlocking` clears `SynchronizationContext`, uses `GetResult` without a dispatcher pump, and must not await env setup inside `StartAsync`. `HostBackgroundController.StartAsync` (Revit and AutoCAD) calls `PythonInitializer.ReserveDebugPort` / `IronPythonInitializer.ReserveDebugPort` on the CAD thread (`DebugPortLease`: named mutex + exclusive `127.0.0.1` bind) so the Execution chips bind immediately, then fire-and-forgets `WhenAll` of both `InitializeAsync` so the **synchronous prefixes** run on that thread: host-owned `InitializeEngine(hostDll)` and IronPython `EnsureEngine` (`CreateEngine` / pyRevit attach). Then **returns without awaiting** pixi/uv/pydevd. `Ipy.CreateEngine` needs sidecar `IronPython.dll` ([0019](../../decisions/0019-ilrepack-and-polyfill-isolated-alc.md)); merging it into the add-in makes `CurrentVersion` throw `IndexOutOfRangeException`. `CreateEngine` must not run after an `await` hop (thread-pool MTA races pythonnet). Import debugpy/pydevd while the lease is held; listen after the placeholder socket is released. Attach polling is off the UI thread (`DebugEndpoint.Attached`). `OnStartup` can finish; other add-ins and WPF splash `Dispatcher.PushFrame` can run. After runtime init, reload the MCP catalog so Python toolsets are not skipped by the early pipe-server preload.

Any hop **before** `PythonEngine.Initialize()` lands on the thread pool: AV (`InitExt`, no GIL) or deadlock (pending-call / GIL). Host-attach therefore runs `InitializeEngine(hostDll)` before `ResolveProviderAsync`. Do not add `InitExt` / `Py_AddPendingCall`. Init takes the lock with `WaitAsync` (not `Wait(0)`); do not insert other awaits between lock acquire and host `InitializeEngine`. Do not await env setup in `IHostedService.StartAsync` (that re-blocks `OnStartup`). Do not `Dispatcher.PushFrame` around `PythonEngine.Initialize`.
...
`PythonEmbedded` extracts `Parser.py`, `ToolParser.py`, `PytestRunner.py`, setup scripts, and `pixi.toml`. Parser is overwritten every load; `pixi.toml` is not if present. Manifest names are resolved across `DevTools.Execution.dll` and ILRepack-merged host layouts (`AcadDevTool.DevTools.Execution.Resources.scripts.*`). Session `~/.pixi/config.toml` sets `tls-root-certs = "system"` for corporate CAs. Pixi `search` uses exit code only — `--json` with `--limit` can dump multi-MB. CLI capture goes through `ExecuteBufferedAsync` (`RunPixiBufferedAsync` / `RunUvBufferedAsync`). Line `ToDelegate` is forbidden for JSON: `pixi list --json` / `uv pip list --format=json` / `pip list --format=json` is one large line; a line pipe fills (~64KB) and deadlocks `WaitForExit` + `CopyFromAsync`. Do not await that helper on the CAD start thread. `RunPixiAsync` / `RunUvAsync` replay log lines after exit. Package stores list via `GetListJsonAsync` (pixi UI list uses `ListExplicitJson()` on the same buffered helper).
