# 0036 Revit Monitor Link-Element Tokens And Host-Space Zoom
...
Click-to-select is Revit-host monitor behavior. It must stay in
`source/RevitDevTool/Logging/` ([0007](0007-revit-core-and-visualization-boundaries.md),
[host-boundaries](../agents/host-boundaries.md)). Shared `DevTools.Logging` and
ZLogger.Scintilla stay host-neutral.

## Decision

### 1. Identity: three kinds, optional `linkInstanceId` scope

Keep `RevitTokenKind` as **ElementId, UniqueId, IfcGuid**. Do **not** add a
...
Do not mutate the view (no `View3D.SetSectionBox`, no extra transaction) on a
log click. Zoom framing is enough.
...
### 6. Placement and API year

- All new types live under `source/RevitDevTool/Logging/Linkify/` (and
  `Logging/Enums` for the token kind). No Revit API types in `DevTools.*`.
...
- AutoCAD monitor linkify is unchanged.

### 7. Failure policy

Clicks stay silent on miss: no document, invalid token, unloaded link,
deleted element, empty bounding box, zoom API throw. Do not show a message
box. Do not guess a host element for a linked token.
...
5. **Create a 3D section box on click.** Rejected — it writes the document,
   needs a 3D view, and is surprising for a monitor hyperlink.
...
7. **Put presenter logic in `RevitDevTool.Core`.** Rejected for this change —
   only the monitor click path needs it. Extract later if a second caller
   appears.
8. **Teach ZLogger.Scintilla `ICustomSerializer` to format `LinkElementId`.**
...
9. **Colon pair (`8821:445566`).** Rejected — `:` is a monitor token
   separator, so the linkifier would see `8821` and `445566` as two tokens.
10. **Shape-match every integer from `Trace` as `ElementId`.** Rejected —
    timestamps, counts, and years would all become clicks. `Trace` / `print`
    use UniqueId, IFC GUID, or `{instanceId}@{inner}`.
11. **Fourth `RevitTokenKind.LinkElementId`.** Rejected — the inner token is
...
- Shared logging stays host-neutral.
...
- Landed in `source/RevitDevTool/Logging/Linkify/` (`LinkToken`,
  `ElementSearcher`, `ElementSelector`, `LinkBoundingBox`).
- Live-host checklist passed 2026-09-17. Plan:
