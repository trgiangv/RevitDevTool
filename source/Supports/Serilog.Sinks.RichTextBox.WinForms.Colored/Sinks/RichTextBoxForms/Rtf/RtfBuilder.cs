...
public sealed class RtfBuilder : IRtfCanvas, IDisposable
{
    ...
    public void AppendHyperlink(string displayText, string href)
    {
        EnsureColorSwitch();
        _body.Append(@"{\field{\*\fldinst HYPERLINK """);
        EscapeAndAppendFieldInstruction(href);
        _body.Append(@"""}{\fldrslt ");
        EscapeAndAppend(displayText);
        _body.Append("}}");
    ...
    private void EscapeAndAppendFieldInstruction(string value)
    {
        if (string.IsNullOrEmpty(value))
        {
            return;
