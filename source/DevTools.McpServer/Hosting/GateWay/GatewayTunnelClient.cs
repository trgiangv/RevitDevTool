    ...
    ILogger logger) : IAsyncDisposable
{
    ...
    private async Task ConnectAndServeAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            await using var server = ModelContextProtocol.Server.McpServer.Create(
                new StreamServerTransport(inbound, outbound, "GatewayTunnel", loggerFactory),
                serverOptions);
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        if (ws is { State: WebSocketState.Open })
        {
            ...
            {
                await ws.CloseAsync(
                    WebSocketCloseStatus.NormalClosure, "Shutdown", CancellationToken.None).ConfigureAwait(false);
            }
            catch { /* best-effort */ }
        }
        ws?.Dispose();
