...
public sealed class LaunchRevitTool : McpServerTool
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(version))
            return ValueTask.FromResult(ErrorResult("versionNumber is required."));

        var revitPath = FindRevitPath(version);
        if (revitPath is null)
            return ValueTask.FromResult(ErrorResult($"Revit {version} installation not found."));
        ...
        try
        {
            ...
            if (process is null)
                return ValueTask.FromResult(ErrorResult("Failed to start Revit process."));
        ...
        {
            return ValueTask.FromResult(ErrorResult($"Failed to launch Revit: {ex.Message}"));
        }
    }
