...
internal static class LocalNUnitTestDiscoverer
{
    public static IReadOnlyList<RemoteTestCase> Discover(string assemblyPath)
    {
        ...
        if (!File.Exists(assemblyPath))
            return Array.Empty<RemoteTestCase>();
        ...
        return DiscoverFromMetadata(peReader.GetMetadataReader(), assemblyPath);
    }

    private static IReadOnlyList<RemoteTestCase> DiscoverFromMetadata(MetadataReader metadata, string assemblyPath)
    {
        var tests = new List<RemoteTestCase>();
        foreach (var typeHandle in metadata.TypeDefinitions)
            AddTypeTests(metadata, typeHandle, assemblyPath, tests);
