...
public sealed class AsyncEventHandler : ExternalEventHandler
{
    ...
    /// <summary>
    ///     Instructing Revit to queue a handler, raise (signal) the external event and async awaiting its completion.
    /// </summary>
    /// <remarks>
    ...
    {
        if (Application.IsInRevitApiMode)
        {
            handler.Invoke(Application.RevitUiApplication);
            return;
        }
