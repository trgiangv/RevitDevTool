...
/// <summary>
/// Tracks current process memory usage in realtime.
/// </summary>
public partial class MemoryViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    private string _lastOperationName = "N/A";

    [ObservableProperty]
    private string _lastOperationProvider = "-";

    [ObservableProperty]
    private bool _lastOperationSuccess;

    [ObservableProperty]
    private long _lastOperationDurationMs;

    [ObservableProperty]
    private double _lastDeltaPrivateMb;

    [ObservableProperty]
    private double _lastDeltaManagedMb;

    [ObservableProperty]
    private int _lastDeltaHandles;

    [ObservableProperty]
    private double _lastPeakPrivateDeltaMb;

    [ObservableProperty]
    private string _lastOutcomeText = "N/A";

    [ObservableProperty]
    private Brush _lastOutcomeBrush = NeutralBrush;

    [ObservableProperty]
    private Brush _lastDeltaPrivateBrush = NeutralBrush;

    [ObservableProperty]
    private Brush _lastDeltaManagedBrush = NeutralBrush;

    [ObservableProperty]
    private Brush _lastDeltaHandlesBrush = NeutralBrush;
    ...
    private void CompleteOperation(bool success, long durationMs)
    {
        ...
        var privateJudgement = EvaluateDeltaMb(LastDeltaPrivateMb, warning: 30, critical: 80);
        var managedJudgement = EvaluateDeltaMb(LastDeltaManagedMb, warning: 20, critical: 50);
        var handlesJudgement = EvaluateDeltaInt(LastDeltaHandles, warning: 20, critical: 60);

        LastDeltaPrivateBrush = ToBrush(privateJudgement);
        LastDeltaManagedBrush = ToBrush(managedJudgement);
        LastDeltaHandlesBrush = ToBrush(handlesJudgement);

        _activeSession = null;
        ApplyRealtimeSnapshot(after);
    }

    private static Brush ToBrush(MemoryJudgement judgement)
    {
    ...
    {
        if (delta >= critical)
            return MemoryJudgement.Critical;
        if (delta >= warning)
            return MemoryJudgement.Warning;
        return MemoryJudgement.Good;
    }
    ...
    private readonly record struct OperationSession(
        string Provider,
        string Target,
