...
public partial class ExecutionViewModel(CommandViewModel commandViewModel, PackageViewModel packageViewModel) : ObservableObject
{
    ...
    {
        CommandViewModel.ExecuteLastItem();
    }
    ...
    private void ExpandAll()
    {
        if (IsPackageMode)
        {
            PackageViewModel.ExpandAllCommand.Execute(null);
        }
        else
        {
            CommandViewModel.ExpandAllCommand.Execute(null);
        }
    }
    ...
    private void CollapseAll()
    {
        if (IsPackageMode)
        {
            PackageViewModel.CollapseAllCommand.Execute(null);
        }
        else
        {
            CommandViewModel.CollapseAllCommand.Execute(null);
        }
    }
    ...
    private void ToggleAll()
    {
        if (IsPackageMode)
        {
            PackageViewModel.ToggleAllCommand.Execute(null);
        }
        else
        {
            CommandViewModel.ToggleAllCommand.Execute(null);
        }
    }
}
...
internal static class ExecutionTreeViewHelper
{
    ...
    public static bool FilterNodeRecursive(TreeNodeBase node, string searchText, bool isDarkTheme)
    {
        ...
        node.HighlightRange = currentMatches
            ? new HighlightRange(index, index + search.Length)
            {
