...
def register_crud_tools(mcp: FastMCP) -> None:
    ...
    async def revit_place_family(
        ...
        type_name: Annotated[str | None, Field(alias="typeName")] = None,
    ...
    async def revit_rotate_elements(
        element_ids: Annotated[list[int], Field(alias="elementIds")],
        axis_origin: Annotated[list[float], Field(alias="axisOrigin", min_length=3, max_length=3)],
        axis_direction: Annotated[list[float], Field(alias="axisDirection", min_length=3, max_length=3)],
    ...
    async def revit_delete_elements(
        element_ids: Annotated[list[int], Field(alias="elementIds")],
        dry_run: Annotated[bool, Field(alias="dryRun")] = False,
    ) -> DeleteElementsResult:
        """Delete elements with optional dry-run preview."""
        return service.delete_elements(element_ids, dry_run)
    ...
    async def revit_clone_parameters(
        ...
        target_ids: Annotated[list[int], Field(alias="targetIds")],
        param_names: Annotated[list[str], Field(alias="paramNames")],
    ) -> CloneParametersResult:
    ...
    async def revit_swap_type(
        element_ids: Annotated[list[int], Field(alias="elementIds")],
        new_type_id: Annotated[int, Field(alias="newTypeId")],
    ) -> SwapTypeResult:
        """Change element type."""
