...
def _resolve_domain(task: str, domain: str | None) -> str:
    ...
    if any(k in t for k in ("duct", "pipe", "mep", "conduit", "hvac")):
        return "mep"
    if any(k in t for k in ("sheet", "view", "schedule", "documentation", "titleblock")):
...
def register_toolset_prompts(mcp) -> None:
    ...
    async def revit_batch_operation(operation: str, criteria: str, updates: str | None = None) -> str:
    ...
    async def revit_undo_recovery(failed_tool: str, error_context: str) -> str:
        ...
        ).format(failed_tool, error_context)
    ...
    async def revit_god_tool_decision(task: str) -> str:
        ...
        t = task.lower()
        non_goals = ("wall", "floor", "roof", "stair", "beam", "curtain", "ifc", "rebar")
