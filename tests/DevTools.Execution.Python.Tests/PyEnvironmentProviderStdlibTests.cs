...
public sealed class PyEnvironmentProviderStdlibTests
{
    ...
    public void TryReadPyvenvHome_ReadsHomeLine()
    {
        ...
        try
        {
            ...
            Assert.IsTrue(PyEnvironmentProvider.TryReadPyvenvHome(cfg, out var home));
            Assert.AreEqual(
    ...
    {
        Assert.IsFalse(PyEnvironmentProvider.TryReadPyvenvHome(
            Path.Combine(Path.GetTempPath(), "no-such-pyvenv.cfg"),
            out var home));
        Assert.AreEqual(string.Empty, home);
    }
    ...
    public void TryResolveSidecarStdlib_UsesLibAndSiblingDlls()
    {
        ...
        try
        {
            ...
            Assert.IsTrue(PythonDepsManager.TryResolveSidecarStdlib(lib, uvPythonInstallDir: null, out var resolvedLib, out var resolvedDlls));
            Assert.AreEqual(lib, resolvedLib);
            Assert.AreEqual(dlls, resolvedDlls);
        }
        finally
        {
    ...
    public void TryResolveSidecarStdlib_LibWithoutDlls_StillResolvesLib()
    {
        ...
        try
        {
            ...
            Assert.IsTrue(PythonDepsManager.TryResolveSidecarStdlib(lib, uvPythonInstallDir: null, out var resolvedLib, out var resolvedDlls));
            Assert.AreEqual(lib, resolvedLib);
            Assert.AreEqual(string.Empty, resolvedDlls);
        }
        finally
        {
            root.Delete(recursive: true);
    ...
    public void TryResolveSidecarStdlib_ScansUvPythonWhenStdlibMissing()
    {
        ...
        try
        {
            ...
            Assert.IsTrue(PythonDepsManager.TryResolveSidecarStdlib(
