...
public sealed class IronPythonPytestRunningTests
{
    ...
    public void SetupRevit_SkipsPrintHijackWhenPytestRunning()
    {
        var setupPath = Path.Combine(
            ...
            "scripts",
            "SetupRevit.py");
        Assert.IsTrue(File.Exists(setupPath), $"SetupRevit.py not found at '{setupPath}'.");
        ...
        Assert.AreNotEqual("custom_print", builtinsWithFlag.print.__name__);
        ...
        Assert.AreEqual("custom_print", builtinsWithoutFlag.print.__name__);
    }
