...
public sealed class CSharpExecutionStrategyTests
{
    ...
    public async Task ExecuteAsync_ValidScript_RunsThroughHostContext()
    {
        ...
        try
        {
            ...
            Assert.IsTrue(messages.Any(message => message.Contains("run_script.csx", StringComparison.Ordinal)));
        }
        finally
        {
    ...
    public async Task ExecuteAsync_CompilationFailure_ReturnsFailedResult()
    {
        ...
        await File.WriteAllTextAsync(scriptPath, "public class {", TestContext.CancellationToken);
