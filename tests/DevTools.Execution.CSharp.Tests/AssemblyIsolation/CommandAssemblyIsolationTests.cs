...
public sealed class CommandAssemblyIsolationTests
{
    ...
    public void Command_plan_returns_the_explicit_parent_contract_instance()
    {
        ...
        Assert.AreSame(parent, session.LoadEntryAssembly());
    }
    ...
    public void Command_plan_resolves_a_transitive_sibling_graph()
    {
        ...
        Assert.AreEqual("transitive", value);
    }
    ...
    public void Command_plan_keeps_conflicting_private_dependency_versions_in_their_own_sessions()
    {
        ...
        Assert.AreEqual("one", InvokeEntry(firstSession));
        Assert.AreEqual("two", InvokeEntry(secondSession));
    }
    ...
    public void Command_plan_reuses_official_wpf_ui_packages_from_the_default_context()
    {
        ...
        Assert.IsTrue(plan.TryShare(new AssemblyName("MahApps.Metro"), out var bound));
        Assert.AreEqual("MahApps.Metro", bound.GetName().Name);

        using var session = AssemblyIsolationSession.Create(plan);
        var value = (string)session.LoadEntryAssembly().GetType("Fixture.Entry")!
            .GetMethod("Value")!.Invoke(null, null)!;

        Assert.AreEqual("official", value);
        Assert.AreSame(AssemblyLoadContext.Default, AssemblyLoadContext.GetLoadContext(bound));
        Assert.AreNotSame(AssemblyLoadContext.GetLoadContext(session.LoadEntryAssembly()), AssemblyLoadContext.GetLoadContext(bound));

        var again = CommandIsolationPlan.Create(graph.EntryPath, Array.Empty<Assembly>());
        Assert.IsTrue(again.TryShare(new AssemblyName("MahApps.Metro"), out var rebound));
        Assert.AreSame(bound, rebound);
    }
    ...
    public void Command_plan_does_not_share_devtools_forked_wpf_ui_assemblies()
    {
        ...
        Assert.IsFalse(plan.TryShare(new AssemblyName("DevTools.MahApps.Metro"), out _));
        Assert.IsTrue(plan.ManagedSources.Any(source => source.Resolve(new AssemblyName("DevTools.MahApps.Metro")) is not null));
    }
