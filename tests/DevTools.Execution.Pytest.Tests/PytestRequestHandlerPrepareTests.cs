...
public sealed class PytestRequestHandlerPrepareTests
{
    ...
    public async Task HandleRunAsync_WhenPythonCannotInitialize_ReturnsPrepareError()
    {
        ...
        await File.WriteAllTextAsync(testFile, "def test_x(): pass", TestContext.CancellationToken);
        ...
        try
        {
            ...
            Assert.IsTrue(body.CollectionErrors.Any(error => error.Message.Contains("[prepare]", StringComparison.Ordinal)));
        }
        finally
        {
