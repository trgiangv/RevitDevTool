...
public class PytestBridgeFramingTests
{
    ...
    public async Task BridgePipeConnection_RoundTripsLengthPrefixedFrames()
    {
        ...
        Assert.AreEqual(body.Length, BitConverter.ToInt32(frame, 0));
        Assert.AreNotEqual((byte)'{', frame[0]);
        ...
        await writer.WriteAsync(request, TestContext.CancellationToken);

        var received = await tcs.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.CancellationToken);
        Assert.AreEqual("42", received.Id);
        Assert.AreEqual("instance/info", received.Method);
        Assert.AreEqual(BridgeMessage.TypeRequest, received.Type);
    }
