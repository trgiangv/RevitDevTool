...
public sealed class IpyTestDriverIoPathsTests
{
    ...
    public void CreateDriverIoPaths_ScopesFilesToHostProcessId()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(Path.Combine(driverDir, "request_111.json"), first.RequestPath);
            Assert.AreEqual(Path.Combine(driverDir, "result_111.json"), first.ResultPath);
            Assert.AreEqual(Path.Combine(driverDir, "request_222.json"), second.RequestPath);
            Assert.AreEqual(Path.Combine(driverDir, "result_222.json"), second.ResultPath);
        }
        finally
        {
            try
