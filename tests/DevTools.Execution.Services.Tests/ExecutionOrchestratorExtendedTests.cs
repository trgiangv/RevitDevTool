...
public sealed class ExecutionOrchestratorExtendedTests
{
    ...
    public async Task ExecuteAsync_ExecutableNode_RecordsProgressAndTelemetry()
    {
        ...
        var result = await orchestrator.ExecuteAsync(node, TestContext.CancellationToken);

        Assert.IsTrue(result.Success);
        Assert.IsTrue(node.IsLastExecuted);
    ...
    public async Task ExecuteAsync_NonExecutable_ReturnsSkipped()
    {
        ...
        var result = await orchestrator.ExecuteAsync(container, TestContext.CancellationToken);

        Assert.IsFalse(result.Success);
        Assert.AreEqual("Node is not executable.", result.Message);
    }
    ...
    public async Task ReloadAsync_RefreshesDiscoveredRoots()
    {
        ...
        try
        {
            ...
            await orchestrator.ReloadAsync(TestContext.CancellationToken);
    ...
    {
        using var orchestrator = CreateOrchestrator();
        var failed = await orchestrator.LoadSavedPathsAsync(["   ", "missing-path"], TestContext.CancellationToken);
    ...
    public async Task RemoveNodeAndClearAll_UpdateTree()
    {
        ...
        await orchestrator.LoadFromPathAsync(directory, TestContext.CancellationToken);
        var loadedRoot = Assert.IsInstanceOfType<ExecutionNodeRoot>(orchestrator.TreeRoot.Single());
