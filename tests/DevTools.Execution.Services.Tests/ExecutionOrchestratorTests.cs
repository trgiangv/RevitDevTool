...
public sealed class ExecutionOrchestratorTests
{
    ...
    public async Task LoadFromPathAsync_NoProviderCanHandle_ThrowsArgumentException()
    {
        ...
        try
        {
            var ex = await Assert.ThrowsExactlyAsync<ArgumentException>(
                () => orchestrator.LoadFromPathAsync(directory, TestContext.CancellationToken));
    ...
    public async Task LoadFromPathAsync_PathUnderExistingRoot_SkipsReload()
    {
        ...
        try
        {
            ...
            await orchestrator.LoadFromPathAsync(rootDirectory, TestContext.CancellationToken);
            await orchestrator.LoadFromPathAsync(childDirectory, TestContext.CancellationToken);
    ...
    public async Task LoadFromPathAsync_HappyPath_WatchesPathAndRaisesTreeChanged()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(1, treeChangedCount);
            fileWatcher.Verify(
            ...
            Assert.AreEqual(rootDirectory, root.RootPath);
        }
        finally
        {
            Directory.Delete(rootDirectory, recursive: true);
