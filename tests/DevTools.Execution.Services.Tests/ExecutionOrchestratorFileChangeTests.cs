...
public sealed class ExecutionOrchestratorFileChangeTests
{
    ...
    public async Task FileChanged_ModifiedEvent_IsIgnored()
    {
        if (HostUiHelper.HostDispatcher is not null)
            Assert.Inconclusive("Host dispatcher is initialized; file-change handler requires inline dispatch.");
        ...
        try
        {
            await orchestrator.LoadFromPathAsync(directory, TestContext.CancellationToken);
    ...
    public async Task FileChanged_RootDeleted_RemovesRootAndUnwatches()
    {
        ...
        try
        {
            ...
            await WaitForAsync(() => removedPath is not null, TestContext.CancellationToken);

            Assert.AreEqual(directory, removedPath);
            Assert.IsEmpty(orchestrator.TreeRoot);
        }
        finally
        {
            Directory.Delete(directory, recursive: true);
    ...
    public async Task FileChanged_DirectoryStructure_ReloadsAffectedRoot()
    {
        ...
        try
        {
            ...
            await WaitForAsync(() => treeChanged >= 2, TestContext.CancellationToken);

            var root = Assert.IsInstanceOfType<ExecutionNodeRoot>(orchestrator.TreeRoot.Single());
            Assert.AreEqual("exec://second", Assert.IsInstanceOfType<ExecutionNode>(root.Children.Single()).Id);
        }
        finally
        {
            Directory.Delete(directory, recursive: true);
    ...
    public async Task FileChanged_RootRenamed_ReloadsAtNewPath()
    {
        ...
        try
        {
            await orchestrator.LoadFromPathAsync(oldDirectory, TestContext.CancellationToken);
            fileWatcher.Raise(new FileChangedEventArgs
            {
                Path = newDirectory,
            ...
            await WaitForAsync(() => removed is not null, TestContext.CancellationToken);

            Assert.AreEqual(oldDirectory, removed!.RootPath);
            Assert.AreEqual(newDirectory, removed.NewPath);
            var root = Assert.IsInstanceOfType<ExecutionNodeRoot>(orchestrator.TreeRoot.Single());
            Assert.AreEqual(newDirectory, root.RootPath);
        }
        finally
        {
