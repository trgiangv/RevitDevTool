...
public sealed class ManifestNativeAssemblySourceTests
{
    ...
    public void Resolves_native_candidates_by_name_and_file_name()
    {
        ...
        Assert.AreSame(candidate, source.Resolve("native-tool.dll"));
        Assert.AreSame(candidate, source.Resolve("native-tool"));
        Assert.AreSame(candidate, source.Resolve("NATIVE-TOOL.DLL"));
    }
    ...
    public void Rejects_ambiguous_native_candidates_with_the_same_file_name()
    {
        ...
        var exception = Assert.ThrowsExactly<InvalidOperationException>(() => new ManifestNativeAssemblySource([first, second]));
