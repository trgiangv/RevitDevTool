...
public sealed class NUnitIdentityIndexTests
{
    ...
    public void Resolve_published_uid_is_a_leaf()
    {
        ...
        Assert.Equal(NUnitIdentityKind.Leaf, hit.Kind);
        Assert.Equal(leaf.TestId, Assert.Single(hit.Leaves).TestId);
    }
    ...
    public void Resolve_method_and_empty_parens_are_the_group()
    {
        ...
        Assert.Equal(NUnitIdentityKind.Group, index.Resolve("Ns.Box.Bottom_corners_share_min_z").Kind);
        Assert.Equal(NUnitIdentityKind.Group, index.Resolve("Ns.Box.Bottom_corners_share_min_z()").Kind);
        Assert.Equal(2, index.Select(["Ns.Box.Bottom_corners_share_min_z()"]).Count);
    }
    ...
    public void Resolve_bare_method_name_is_unknown()
    {
        ...
        Assert.Equal(NUnitIdentityKind.Unknown, hit.Kind);
        Assert.Empty(hit.Leaves);
    }
