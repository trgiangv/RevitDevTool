...
public sealed class FSharpDependencyResolverExtendedTests
{
    ...
    public async Task ResolveAsync_WithNugetPackage_AddsPackageDlls()
    {
        ...
        try
        {
            ...
            Assert.IsNotEmpty(resolution.References);
            Assert.Contains(
                path => Path.GetFileName(path).Equals("Newtonsoft.Json.dll", StringComparison.OrdinalIgnoreCase),
    ...
    public async Task ResolveAsync_WithHostReference_AddsResolvedDll()
    {
        ...
        await File.WriteAllBytesAsync(dependencyPath, [0x4D, 0x5A], TestContext.CancellationToken);
    ...
    public async Task ResolveAsync_RewrittenGraph_EmitsLineDirectiveToOriginalScript()
    {
        ...
        try
        {
            ...
            var rewritten = await File.ReadAllLinesAsync(resolution.ScriptPath, TestContext.CancellationToken);
            ...
            Assert.IsTrue(rewritten.Contains("let x = 1"));
    ...
    public async Task ResolveAsync_RewritesHostYear_ViaBridge()
    {
        ...
        await File.WriteAllBytesAsync(oldDll, [0x4D, 0x5A], TestContext.CancellationToken);
        await File.WriteAllBytesAsync(newDll, [0x4D, 0x5A], TestContext.CancellationToken);
    ...
    public async Task ResolveAsync_WithConflictingNugetVersions_Throws()
    {
        ...
        await File.WriteAllTextAsync(first, "#r \"nuget: Newtonsoft.Json, 13.0.1\"", TestContext.CancellationToken);
        await File.WriteAllTextAsync(second, "#r \"nuget: Newtonsoft.Json, 13.0.3\"", TestContext.CancellationToken);
        await File.WriteAllTextAsync(entry, $"#load @\"{first.Replace('\\', '/')}\"\n#load @\"{second.Replace('\\', '/')}\"", TestContext.CancellationToken);

        var graph = await FSharpScriptGraph.BuildLoadGraphAsync(entry, TestContext.CancellationToken);
        var resolver = new FSharpDependencyResolver(
            NullLogger<FSharpDependencyResolver>.Instance,
            new NugetManager(NullLogger<NugetManager>.Instance));
        ...
        try
        {
            var ex = await Assert.ThrowsExactlyAsync<InvalidOperationException>(
                () => resolver.ResolveAsync(entry, graph, ExecutionTestHelpers.CreateScriptBridge(), ct: TestContext.CancellationToken));
    ...
    public async Task ResolveAsync_ReportsProgressForPackageResolution()
    {
        ...
        await File.WriteAllTextAsync(scriptPath, "#r \"nuget: Newtonsoft.Json, 13.0.3\"", TestContext.CancellationToken);
        var graph = await FSharpScriptGraph.BuildLoadGraphAsync(scriptPath, TestContext.CancellationToken);
        var resolver = new FSharpDependencyResolver(
            NullLogger<FSharpDependencyResolver>.Instance,
            new NugetManager(NullLogger<NugetManager>.Instance));
        ...
        try
        {
            ...
            Assert.Contains(message => message.Contains("NuGet", StringComparison.OrdinalIgnoreCase), messages);
            resolution.Cleanup?.Dispose();
        }
        finally
        {
